import { useState, useMemo } from 'react';
import ActivityCard from '../ActivityCard.jsx';
import { BASE_ACTIVITIES, STICKY_DEFAULTS } from '../../data/activities.js';
import { SOURCED_EVENTS } from '../../data/events.js';
import { getSeason, wxInfo, scoreActivity } from '../../utils/weather.js';
import { getWeekend, fmtShort } from '../../utils/date.js';

const CAT_FILTERS = [
  { id: 'all',      label: 'All',          e: '✨' },
  { id: 'sourced',  label: 'This Weekend', e: '📰' },
  { id: 'outdoor',  label: 'Outdoor',      e: '🌿' },
  { id: 'indoor',   label: 'Indoor',       e: '🏛️' },
  { id: 'theater',  label: 'Theater',      e: '🎭' },
  { id: 'food',     label: 'Food',         e: '🍺' },
  { id: 'seasonal', label: 'Annual',       e: '🎡' },
  { id: 'sticky',   label: 'Favourites',   e: '⭐' },
];
const LOC_FILTERS = [
  { id: 'all',      label: 'All locations' },
  { id: 'munich',   label: '🏙️ In Munich'  },
  { id: 'day-trip', label: '🚗 Day trips'   },
];
const TYPE_FILTERS = [
  { id: 'all',      label: 'All types'          },
  { id: 'sourced',  label: '📰 This weekend'    },
  { id: 'venue',    label: '📌 All-year venues' },
  { id: 'seasonal', label: '🎡 Annual festivals'},
];

export default function ExplorerTab({ weather, weekendPlan, setWeekendPlan, stickyActivities, setStickyActivities }) {
  const [catFilter,      setCatFilter]      = useState('all');
  const [locationFilter, setLocationFilter] = useState('all');
  const [typeFilter,     setTypeFilter]     = useState('all');
  const [showAddSticky,  setShowAddSticky]  = useState(false);
  const [newSticky,      setNewSticky]      = useState({ name: '', emoji: '⭐', desc: '' });

  const season = getSeason(new Date().getMonth() + 1);
  const wxCat  = weather ? wxInfo(weather.sat.code).cat : 'sunny';

  const allActivities = useMemo(() => {
    const userStickies = stickyActivities.filter(s => !STICKY_DEFAULTS.find(d => d.id === s.id));
    return [...STICKY_DEFAULTS, ...userStickies, ...BASE_ACTIVITIES];
  }, [stickyActivities]);

  const allWithSourced = useMemo(() => [...SOURCED_EVENTS, ...allActivities], [allActivities]);

  const scored = useMemo(() =>
    allWithSourced
      .map(a => ({ ...a, _score: scoreActivity(a, wxCat, season) }))
      .sort((a, b) => b._score - a._score),
    [allWithSourced, wxCat, season]
  );

  const filtered = useMemo(() => {
    let list = scored;
    if (catFilter !== 'all') {
      if (catFilter === 'sticky')       list = list.filter(a => a.cat === 'sticky');
      else if (catFilter === 'sourced') list = list.filter(a => a.eventType === 'sourced');
      else                              list = list.filter(a => a.cat === catFilter && a.eventType !== 'sourced');
    }
    if (locationFilter !== 'all') list = list.filter(a => a.location === locationFilter);
    if (typeFilter === 'sourced')  list = list.filter(a => a.eventType === 'sourced');
    if (typeFilter === 'venue')    list = list.filter(a => a.eventType === 'venue');
    if (typeFilter === 'seasonal') list = list.filter(a => a.eventType === 'seasonal');
    return list;
  }, [scored, catFilter, locationFilter, typeFilter]);

  const isAdded  = (actId, day) => (weekendPlan[day] || []).some(a => a.id === actId);
  const addToDay = (act, day)   => setWeekendPlan(p => ({ ...p, [day]: [...(p[day] || []), { ...act, _key: act.id + Date.now() }] }));

  const sourcedItems  = filtered.filter(a => a.eventType === 'sourced');
  const stickyItems   = filtered.filter(a => a.cat === 'sticky');
  const regularItems  = filtered.filter(a => a.eventType !== 'sourced' && a.cat !== 'sticky');

  const saveSticky = () => {
    if (!newSticky.name.trim()) return;
    const s = {
      id: 'user_sticky_' + Date.now(),
      name: newSticky.name.trim(),
      emoji: newSticky.emoji,
      desc: newSticky.desc,
      cat: 'sticky', location: 'munich', eventType: 'venue',
      weather: ['any'], season: ['all'], duration: '', age: '', url: '', tags: ['personal'],
    };
    setStickyActivities([...stickyActivities, s]);
    setNewSticky({ name: '', emoji: '⭐', desc: '' });
    setShowAddSticky(false);
  };

  const removeSticky = id => setStickyActivities(stickyActivities.filter(s => s.id !== id));

  const { sat, sun } = getWeekend();

  return (
    <div className="fade-in space-y-4">
      {/* header row */}
      <div className="flex items-start justify-between gap-2">
        <div>
          <h2 className="text-lg font-bold text-stone-800">Explorer</h2>
          <p className="text-xs text-stone-400 mt-0.5">
            {filtered.length} {filtered.length === 1 ? 'activity' : 'activities'} · ranked for {wxCat === 'rainy' ? '🌧️ rainy' : '☀️ ' + wxCat} {season}
          </p>
        </div>
        {weather && (
          <div className="flex-shrink-0 text-right text-xs text-stone-500 bg-white rounded-xl px-3 py-2 border border-stone-200">
            <div className="text-[10px] text-stone-400 font-bold uppercase tracking-wide mb-0.5">Weekend</div>
            <div>{wxInfo(weather.sat.code).emoji} Sa {weather.sat.maxT}° · {wxInfo(weather.sun.code).emoji} So {weather.sun.maxT}°</div>
          </div>
        )}
      </div>

      {/* FILTER ROW 1 — category (horizontal scroll on mobile) */}
      <div className="flex gap-1.5 scroll-x -mx-4 px-4 pb-1">
        {CAT_FILTERS.map(f => (
          <button
            key={f.id}
            onClick={() => setCatFilter(f.id)}
            className={`flex-shrink-0 text-xs px-3 py-2 rounded-full font-semibold border transition-colors min-h-[36px] ${
              catFilter === f.id
                ? 'bg-stone-800 text-white border-stone-800'
                : 'bg-white text-stone-600 border-stone-200'
            }`}
          >
            {f.e} {f.label}
          </button>
        ))}
      </div>

      {/* FILTER ROW 2 — location + type */}
      <div className="flex gap-1.5 scroll-x -mx-4 px-4 pb-1">
        {LOC_FILTERS.map(f => (
          <button
            key={f.id}
            onClick={() => setLocationFilter(f.id)}
            className={`flex-shrink-0 text-[11px] px-2.5 py-1.5 rounded-full font-semibold border transition-colors min-h-[32px] ${
              locationFilter === f.id
                ? 'bg-sky-600 text-white border-sky-600'
                : 'bg-white text-stone-500 border-stone-200'
            }`}
          >
            {f.label}
          </button>
        ))}
        <div className="w-px h-6 bg-stone-200 flex-shrink-0 self-center mx-1" />
        {TYPE_FILTERS.map(f => (
          <button
            key={f.id}
            onClick={() => setTypeFilter(f.id)}
            className={`flex-shrink-0 text-[11px] px-2.5 py-1.5 rounded-full font-semibold border transition-colors min-h-[32px] ${
              typeFilter === f.id
                ? 'bg-violet-600 text-white border-violet-600'
                : 'bg-white text-stone-500 border-stone-200'
            }`}
          >
            {f.label}
          </button>
        ))}
      </div>

      {/* SOURCED EVENTS */}
      {sourcedItems.length > 0 && (
        <section>
          <div className="flex items-center gap-2 mb-2">
            <div className="text-xs font-bold text-violet-700 uppercase tracking-wide">📰 This Weekend's Events</div>
            <div className="text-[10px] text-stone-400 bg-stone-100 px-2 py-0.5 rounded-full">{fmtShort(sat)} – {fmtShort(sun)}</div>
            <div className="text-[9px] text-stone-300 ml-auto hidden sm:block">sourced via web search · refresh each Friday</div>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {sourcedItems.map(act => (
              <ActivityCard key={act.id} act={act} wxCat={wxCat}
                onAddSat={() => addToDay(act, 'sat')} onAddSun={() => addToDay(act, 'sun')}
                addedSat={isAdded(act.id, 'sat')} addedSun={isAdded(act.id, 'sun')} />
            ))}
          </div>
          {(stickyItems.length > 0 || regularItems.length > 0) && typeFilter === 'all' && catFilter === 'all' && (
            <div className="border-t border-stone-100 mt-4" />
          )}
        </section>
      )}

      {/* STICKY FAVOURITES */}
      {stickyItems.length > 0 && (typeFilter === 'all' || typeFilter === 'venue') && (
        <section>
          <div className="flex items-center justify-between mb-2">
            <div className="text-xs font-bold text-stone-400 uppercase tracking-wide">⭐ Sticky Favourites</div>
            <button onClick={() => setShowAddSticky(true)} className="text-xs text-amber-600 font-semibold min-h-[36px] px-2">
              + Add yours
            </button>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {stickyItems.map(act => (
              <div key={act.id} className="relative">
                <ActivityCard act={act} wxCat={wxCat}
                  onAddSat={() => addToDay(act, 'sat')} onAddSun={() => addToDay(act, 'sun')}
                  addedSat={isAdded(act.id, 'sat')} addedSun={isAdded(act.id, 'sun')} />
                {stickyActivities.find(s => s.id === act.id) && (
                  <button
                    onClick={() => removeSticky(act.id)}
                    className="absolute top-2 left-2 text-[9px] text-red-400 bg-white rounded px-1 border border-red-100 min-h-[24px]"
                  >
                    remove
                  </button>
                )}
              </div>
            ))}
          </div>

          {showAddSticky && (
            <div className="mt-3 bg-rose-50 border border-rose-200 rounded-xl p-4 space-y-3">
              <div className="font-semibold text-rose-700 text-sm">Add a Sticky Activity</div>
              <div className="flex gap-2">
                <input
                  value={newSticky.emoji}
                  onChange={e => setNewSticky(p => ({ ...p, emoji: e.target.value }))}
                  className="w-12 border border-stone-200 rounded-lg px-2 py-2 text-center text-lg"
                />
                <input
                  value={newSticky.name}
                  onChange={e => setNewSticky(p => ({ ...p, name: e.target.value }))}
                  className="flex-1 border border-stone-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-amber-400"
                  placeholder="Activity name"
                />
              </div>
              <input
                value={newSticky.desc}
                onChange={e => setNewSticky(p => ({ ...p, desc: e.target.value }))}
                className="w-full border border-stone-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-amber-400"
                placeholder="Short description (optional)"
              />
              <div className="flex gap-2">
                <button onClick={() => setShowAddSticky(false)} className="flex-1 bg-white text-stone-600 border border-stone-200 rounded-lg py-2.5 text-sm font-semibold">
                  Cancel
                </button>
                <button onClick={saveSticky} disabled={!newSticky.name.trim()} className="flex-1 bg-rose-500 disabled:bg-stone-200 text-white rounded-lg py-2.5 text-sm font-bold">
                  Save
                </button>
              </div>
            </div>
          )}
          {regularItems.length > 0 && <div className="border-t border-stone-100 mt-4" />}
        </section>
      )}

      {/* add sticky shortcut when section is hidden */}
      {stickyItems.length === 0 && catFilter !== 'sticky' && typeFilter === 'all' && locationFilter === 'all' && (
        <button onClick={() => setShowAddSticky(true)} className="text-xs text-amber-600 font-semibold">
          ⭐ Add sticky favourite
        </button>
      )}

      {/* ALL OTHER ACTIVITIES */}
      {regularItems.length > 0 && (
        <section>
          {(sourcedItems.length > 0 || stickyItems.length > 0) && typeFilter === 'all' && catFilter === 'all' && (
            <div className="text-xs font-bold text-stone-400 uppercase tracking-wide mb-3">📌 All-Year Activities — ranked by fit</div>
          )}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {regularItems.map(act => (
              <ActivityCard key={act.id} act={act} wxCat={wxCat}
                onAddSat={() => addToDay(act, 'sat')} onAddSun={() => addToDay(act, 'sun')}
                addedSat={isAdded(act.id, 'sat')} addedSun={isAdded(act.id, 'sun')} />
            ))}
          </div>
        </section>
      )}

      {/* empty state */}
      {filtered.length === 0 && (
        <div className="text-center py-12 text-stone-400">
          <div className="text-4xl mb-3">🔍</div>
          <div className="font-semibold">No activities match these filters</div>
          <button
            onClick={() => { setCatFilter('all'); setLocationFilter('all'); setTypeFilter('all'); }}
            className="mt-3 text-xs text-amber-600 hover:underline"
          >
            Clear all filters
          </button>
        </div>
      )}
    </div>
  );
}
