...
export default function WxCard({ label, data, loading, error }) {
  ...
  return (
    <div className={`bg-white rounded-2xl p-4 border shadow-sm ${wx.cat === 'rainy' ? 'border-blue-200' : 'border-stone-200'}`}>
      ...
      <div className="text-xs text-stone-500 mb-2">{fmtLong(data.date, lang)}</div>
      ...
      <div className="text-xs font-semibold text-stone-600">{t(`wx.${wx.labelKey}`)}</div>
      {data.precip > 0 && (
